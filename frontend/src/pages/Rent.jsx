...
const Rent = () => {
  ...
  const propertydata = () =>{
    ...
    if(selectedLocation!== '') {
      pcopy=pcopy.filter(p=>p.location.includes(selectedLocation));
    }

    if(bhkType.length>0) {
      pcopy=pcopy.filter(p=>bhkType.includes(p.bhk))
    }
  ...
  return (
    ...
    <div className='px-3 sm:px-10 flex flex-col sm:flex-row gap-2 sm:gap-10'>
      <div className='min-w-60'>
        ...
        <div className={`shadow-md hover:shadow-xl border border-slate-300 bg-slate-100 pl-5 py-5 mt-6 ${showFilter ? '' : 'hidden' } sm:block rounded  `} >
          <p className='text-lg mb-3 font-medium'>BHK Type</p>
          <div className='flex flex-col gap-2 text-sm font-light'>
            <p className='flex gap-2 rounded-4xl'> 
              <input type='checkbox' value='1BHK' className='cursor-pointer' onChange={togglebhktype} />1BHK
            </p>
            <p className='flex gap-2'>
              <input type='checkbox' value='2BHK' className='cursor-pointer' onChange={togglebhktype} />2BHK
            </p>
            <p className='flex gap-2'>
              <input type='checkbox' value='3BHK' className='cursor-pointer' onChange={togglebhktype} />3BHK
            </p>
          </div>
        </div>

        <div className={`shadow-md hover:shadow-xl border border-slate-300 bg-slate-100 sm:block pl-5 py-5 mt-6 ${showFilter ? '': 'hidden' } `}>
          <p className='text-lg font-medium mb-3'>Price Range</p>
          <div className='flex flex-col gap-2 text-sm font-light'>
            <p className='flex gap-2'>
              <input type='radio' value='below10k' name='price' onChange={togglepricerange} /> Below {currency}10,000
            </p>
              <p className='flex gap-2'>
              <input type='radio' value='10-15' name='price' onChange={togglepricerange} />{currency}10,000-{currency}15,000
            </p>
              <p className='flex gap-2'>
              <input type='radio' value='15-20' name='price' onChange={togglepricerange} />{currency}15,000-{currency}20,000
            </p>
             <p className='flex gap-2'>
              <input type='radio' value='above20' name='price' onChange={togglepricerange} />Above {currency}20,000
            </p>
          </div>
        </div>
      </div> 
